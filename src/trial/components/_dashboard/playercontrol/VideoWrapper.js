...
const CustomCard = ({ styles, cover, video, title, brand, date }) => {
  const mediaStyles = useCoverCardMediaStyles();
  return (
    <L.O className={cx(styles.root, styles.color)} pt={20}>
      <CardMedia image={cover} className={styles.cover} classes={mediaStyles} />
      <L.O className={styles.content} p={2}>
        <L.O position={'relative'} zIndex={1}>
          <Row p={0} gap={2}>
            <Item>
                 {video}
            </Item>
      ...
      >
        <Grid item>
          <CustomCard
            ...
            date={'02.04.2020'}
            cover={
              '/static/video-rental/images/movie_37.png'
            }
